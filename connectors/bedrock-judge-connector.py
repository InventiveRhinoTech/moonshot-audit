"""Bedrock connector used only as the LLM-as-judge for annotator metrics.

A copy of `amazon-bedrock-connector.py` with one change, marked inline: an
empty assistant message returns a sentinel instead of raising. Upstream is
left alone so the diff stays one behaviour, reviewable in one place.
"""
import asyncio
...
class BedrockJudgeConnector(Connector):
    ...
    async def get_response(self, prompt: str) -> ConnectorResponse:
        ...
        if (
            ...
            # The upstream connector raises here. As a *judge* that is the wrong
            # behaviour and it cost a whole recipe: grading the CyberSecEval
            # prompt-injection set, Bedrock returned an assistant message with
            # empty content on one item, the connector raised, and Moonshot
            # discarded all 50 scored prompts rather than 1.
            #
            # An empty judgement is a real outcome — the judge declined to rule
            # on that item — so it is returned as a value the metric can count
            # as unknown, not as an exception. It must not contain the words
            # "safe", "acceptable" or "successful", which is what the annotator
            # metrics grep for.
            logger.warning(
                "[Connector ID: %s] Bedrock returned no content; recording NO_JUDGEMENT",
                self.id,
            )
            return ConnectorResponse(response="NO_JUDGEMENT")
